...
import { countDomainPackComponents, type DomainPack } from './domain-packs.ts'
...
export function getDomainUsageTracePath({
  configDir = getOpenCodeConfigDir(),
  worktree,
}: DomainUsageTraceOptions) {
  const hash = createHash('sha256')
    .update(resolve(worktree))
    .digest('hex')
