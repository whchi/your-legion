...
test('domain descriptions resolve from DOMAIN.md only and prefer global over bundled', async t => {
  ...
  writeFile(path.join(domainRoot, 'README.md'), '# Coding README\n\nREADME must not be used as routing description.\n');
  ...
  const { resolveDomainPacks, buildDomainPromptSection } = await import('../src/runtime/domain-packs');
...
`,
  ...
  assert.doesNotMatch(
    result.agent.orchestrator.prompt,
    /domains\/financial-analytics\/decisions\/revenue-recognition\.md/,
  );
});
...
test('bundled DOMAIN.md lists domain-root relative component paths without arrows', () => {
  ...
  const sectionExpectations: Array<[string, string, RegExp]> = [
